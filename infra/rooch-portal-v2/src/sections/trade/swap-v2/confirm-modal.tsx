...
export default function SwapConfirmModal({
  ...
  open: boolean;
  onClose: () => void;
  fromCoin: TradeCoinType;
  toCoin: TradeCoinType;
  slippage: number;
}) {
  ...
  const handleSwap = () => {
    ...
    const fixedFromCoinAmount = toDust(fromCoin.amount, fromCoin.decimal);
    const fixedToCoinAmount = toDust(toCoin.amount, toCoin.decimal);
    ...
    tx.callFunction({
      ...
      typeArgs: [fromCoin.type, toCoin.type],
    });
          ...
          <CardContent className="!pt-0">
            <Box sx={{ mt: 2 }}>
              <Stack direction="column" spacing="2">
                <Stack direction="row" alignItems="center" justifyContent="space-between">
                  ...
                  <Box className="text-gray-400 text-sm font-medium">
                    {' '}
                    - {fNumber(fromCoin.amount, { maximumFractionDigits: 16 })}
                    {` ${fromCoin.symbol} `}
                  </Box>
                </Stack>
                <Stack
                ...
                >
                  ...
                  <Box className="text-gray-400 text-sm font-medium">
                    {' '}
                    + {fNumber(toCoin.amount, { maximumFractionDigits: 16 })}
                    {` ${toCoin.symbol} `}
                  </Box>
                </Stack>
              </Stack>
              <Stack direction="row" alignItems="center" spacing={0.5} sx={{ mt: 2 }}>
                ...
                <Box>
                  1 {fromCoin.symbol} ≈{' '}
                  {fNumber(BigNumber(toCoin.amount).div(fromCoin.amount).toString(), {
