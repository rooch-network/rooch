                ...
                Ok(inscription_body_opt) => {
                    ...
                    match inscription_body_opt {
                        Some(inscription_body) => {
                            let ord_inscription_info = OrdInscriptionInfo::new(transaction, inscription_body);
                            let rooch_inscription_info_result = rooch_client::query_inscription(config, transaction.txid.clone(), transaction.index.clone());
                            let _ = check_ord_data(&transaction, ord_inscription_info, rooch_inscription_info_result)?;
                        },
                        None => println!(
...
pub fn match_inscription_data(
    ...
    (ord_inscription_info.address == rooch_inscription_info.owner_bitcoin_address)
        && (ord_inscription_info.content_type == rooch_inscription_info.value.content_type)
