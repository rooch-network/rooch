Feature: Rooch CLI integration tests
    ...
    Scenario: account
      ...
      Then cmd: "account list --json"
      Then assert: "'{{$.account[-2].account_address}}' == '{{$.account[-1].account0.hex_address}}'"
      Then assert: "'{{$.account[-2].sequence_number}}' == '0'"
      Then assert: "'{{$.account[-2].bitcoin_address}}' == null"
  ...
  Scenario: indexer
    ...
    Then cmd: "rpc request --method rooch_queryEvents --params '[{"tx_order_range":{"from_order":0, "to_order":2}}, null, "30", {"descending": true,"showDisplay":false}]' --json"
