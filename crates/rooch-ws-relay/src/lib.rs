...
use crate::server::close::Close;
use crate::server::close::CloseCmd;
use crate::server::config::{Settings, VerifiedUsersMode};
use crate::server::conn;
use crate::server::db;
use crate::server::db::SubmittedEvent;
use crate::server::error::{Error, Result};
use crate::server::event::Event;
use crate::server::event::EventCmd;
use crate::server::event::EventWrapper;
use crate::server::info::RelayInfo;
use crate::server::nip05;
use crate::server::notice::Notice;
use crate::payment::handler::InvoiceInfo;
use crate::payment::handler::PaymentMessage;
use crate::server::error::Error::CommandUnknownError;
use crate::server::event::EventWrapper::{WrappedAuth, WrappedEvent};
...
pub fn http_client(url: impl AsRef<str>) -> anyhow::Result<HttpClient> {
...
impl Service {
    ...
    pub fn start(&mut self, settings: Settings, shutdown_rx: MpscReceiver<()>) -> anyhow::Result<()> {
        // run this in a new thread
        let handle = thread::spawn(move || {
            let _svr = start_server(&settings, shutdown_rx);
        });
...
pub fn start_server(settings: &Settings, shutdown_rx: MpscReceiver<()>) -> Result<(), Error> {
    ...
    rt.block_on(async {
        ...
        if settings.pay_to_relay.enabled {
            let payment_opt = payment::handler::Payment::new(
                repo.clone(),
                payment_tx.clone(),
                payment_rx,
                bcast_tx.clone(),
