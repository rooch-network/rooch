...
module rooch_framework::ecdsa_k1_recoverable_validator {
   ...
   public fun ecdsa_k1_recoverable_authentication_key(payload: &vector<u8>): vector<u8> {
      let public_key = ecdsa_k1_recoverable_public_key(payload);
   ...
   public fun validate(ctx: &StorageContext, payload: vector<u8>){
      ...
      // let auth_key = ecdsa_k1_recoverable_authentication_key(&payload);
