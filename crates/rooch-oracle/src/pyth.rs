...
pub struct PythConfig {
    #[arg(long, default_value = "https://hermes.pyth.network/v2/updates/price/latest?ids%5B%5D=0xe62df6c8b4a85fe1a67db44dc12de5db330f7ac66b72dc658afedf0f4a415b43&ids%5B%5D=0xc96458d393fe9deb7a7d63a0ac41e2898a67a7750dbd166673279e06c868df0a")]
    pub pyth_url: String,

    #[arg(long, env = "ROOCH_PYTH_WALLET_DIR", default_value = "~/.rooch/rooch_config")]
...
impl Pyth {
    ...
    pub async fn subscribe(&self, package_id: &str) {
        ...
        let oracle_obj = parse_and_convert(format!("object_id:{}", self.pyth_config.pyth_oracle_id).as_str(), &address_mapping);
        let ticker = parse_and_convert("string:BTCUSD", &address_mapping);
        let identifier = parse_and_convert("string:PYTH", &address_mapping);
        let admin_obj = parse_and_convert(format!("object_id:{}", self.pyth_config.pyth_admin_id).as_str(), &address_mapping);
        while let Some(msg) = rx.recv().await {
            let wallet_state = self.wallet_state.write().await;
            let price = format!("u256:{}", msg["parsed"][0]["ema_price"]["price"].as_str().unwrap());
