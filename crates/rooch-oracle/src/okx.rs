...
pub struct OkxConfig {
    ...
    #[arg(long, env = "ROOCH_OKX_WALLET_DIR", default_value = "~/.rooch/rooch_config")]
...
impl Okx {
    ...
    pub async fn subscribe(&self, package_id: &str) {
        ...
        let oracle_obj = parse_and_convert(format!("object_id:{}", self.okx_config.okx_oracle_id).as_str(), &address_mapping);
        let ticker = parse_and_convert("string:BTCUSD", &address_mapping);
        let identifier = parse_and_convert("string:OKX", &address_mapping);
        let admin_obj = parse_and_convert(format!("object_id:{}", self.okx_config.okx_admin_id).as_str(), &address_mapping);
        ...
        while let Some(msg) = rx.recv().await {
            ...
            if msg_value["data"][0]["last"].as_str().is_none() || Instant::now().duration_since(last_execution) < Duration::from_secs(self.okx_config.okx_submit_interval) {
                continue
            }
            last_execution = Instant::now();
            let price = format!("u256:{}", msg_value["data"][0]["last"].as_str().unwrap().parse::<f64>().unwrap() * 10f64.powi(8));
            ...
            info!("Received Okex price: {:?}", msg_value["data"][0]["last"].as_str().unwrap());
        }
