...
use rand::Rng;
use rooch_types::{
    bitcoin::multisign_account::{self, MultisignAccountModule}, crypto::RoochKeyPair, genesis_config::G_TEST_CONFIG, transaction::RoochTransactionData
};
...
async fn test_genesis_multisign_account() {
    ...
    assert_eq!(multisign_config.multisign_bitcoin_address, genesis_multisign_account);

    let account_module = binding_test.as_module_binding::<MultisignAccountModule>(); 

    let genesis_multisign_account_from_move = account_module.generate_multisign_address(
        multisign_config.threshold,
        multisign_config.participant_public_keys.clone(),
    ).unwrap();

    assert_eq!(genesis_multisign_account, genesis_multisign_account_from_move);
}
...
async fn test_multisign_account_random() {
    ...
    assert_eq!(bitcoin_address_from_rust, bitcoin_address_from_move, "threshold: {}, count: {}, pubkeys:{}", threshold, count
     , pubkeys.iter().map(|pk| hex::encode(pk)).collect::<Vec<_>>().join(","));
}