...
fn test_session_key_ecdsa() {
    ...
    assert!(session_key_option.is_some(), "Session key not found");
    let session_key = session_key_option.unwrap();
    assert_eq!(session_key.scheme, BuiltinScheme::Ecdsa.flag() as u64);
    assert_eq!(session_key.scopes, vec![session_scope]);
    assert_eq!(session_key.expiration_time, expiration_time);
    assert_eq!(session_key.max_inactive_interval, max_inactive_interval);
}
...
#[test]
fn test_session_key_ecdsa_recoverable() {
    let mut binding_test = binding_test::RustBindingTest::new().unwrap();
    ...
    assert_eq!(
        session_key.scheme,
        BuiltinScheme::EcdsaRecoverable.flag() as u64
    );
    assert_eq!(session_key.scopes, vec![session_scope]);
    assert_eq!(session_key.expiration_time, expiration_time);
    assert_eq!(session_key.max_inactive_interval, max_inactive_interval);
...
fn test_session_key_schnorr() {
    ...
    assert!(session_key_option.is_some(), "Session key not found");
    let session_key = session_key_option.unwrap();
    assert_eq!(session_key.scheme, BuiltinScheme::Schnorr.flag() as u64);
    assert_eq!(session_key.scopes, vec![session_scope]);
    assert_eq!(session_key.expiration_time, expiration_time);
    assert_eq!(session_key.max_inactive_interval, max_inactive_interval);
}
