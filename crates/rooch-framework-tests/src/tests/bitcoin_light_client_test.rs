...
use rooch_types::framework::bitcoin_light_client::BlockHeader;
...
fn test_submit_block() {
    ...
    let block_hash = decode.header.block_hash();
    let block_header: BlockHeader = decode.header.into();

    let action = MoveAction::Function(rooch_types::framework::bitcoin_light_client::BitcoinLightClientModule::create_submit_new_block_call(height, block_hash.to_byte_array().to_vec(), &block_header));
    ...
    assert_eq!(
        bitcoin_light_client_module
            .get_block(block_hash.to_byte_array().to_vec())
            .unwrap()
            .unwrap(),
        block_header
    );
