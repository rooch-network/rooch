...
async fn test_reachable_stale_disjoint_basic() {
    ...
    println!(
        "Created tree with {} leaf nodes and root",
        all_node_hashes.len()
    );
    ...
    tree_builder
        .inject_stale_indices(vec![stale_leaf_hash], stale_cutoff_order)
        .unwrap();
...
async fn test_reachable_stale_disjoint_complex() {
    ...
    store
        .get_prune_store()
        .inc_node_refcount(zero_refcount_node)
        .unwrap();
    store
        .get_prune_store()
        .dec_node_refcount(zero_refcount_node)
        .unwrap();
    ...
    store
        .get_prune_store()
        .inc_node_refcount(positive_refcount_node)
        .unwrap();
    ...
    for node_hash in [
        no_refcount_node,
        zero_refcount_node,
        positive_refcount_node,
        root_hash,
    ] {
        let node = Node::new_leaf(
            H256::random(),
            SMTObject::<Vec<u8>>::from_origin(vec![1, 2, 3]).unwrap(),
        );
        node_store.put(node_hash, node.encode().unwrap()).unwrap();
    }

    // Mark some nodes as stale
    let stale_nodes = [no_refcount_node, zero_refcount_node];
    let stale_cutoff_order = 100;
    ...
    for (i, node_hash) in stale_nodes.iter().enumerate() {
        ...
        store
            .get_prune_store()
            .write_stale_indices(tx_order, &[stale_pair])
            .unwrap();
    }
    ...
    println!(
        "BuildReach scanned {} nodes in complex scenario",
        scanned_size
    );
    ...
    for stale_hash in &stale_hashes {
        ...
        assert!(
            ...
            store
                .get_prune_store()
                .get_node_refcount(*stale_hash)
                .unwrap()
        );
    }

    // Verify refcount states
    let no_refcount = store
        .get_prune_store()
        .get_node_refcount(no_refcount_node)
        .unwrap();
    let zero_refcount = store
        .get_prune_store()
        .get_node_refcount(zero_refcount_node)
        .unwrap();
    let positive_refcount = store
        .get_prune_store()
        .get_node_refcount(positive_refcount_node)
        .unwrap();

    println!(
        "Refcount states: no_refcount={:?}, zero_refcount={:?}, positive_refcount={:?}",
        no_refcount, zero_refcount, positive_refcount
    );

    // Validate refcount behavior (the exact behavior may vary)
    assert!(
        positive_refcount == Some(1),
        "Positive refcount should be maintained"
    );
...
async fn test_stale_index_semantic_validation() {
    ...
    store
        .get_prune_store()
        .write_stale_indices(tx_order_1, &[stale1])
        .unwrap();
    store
        .get_prune_store()
        .write_stale_indices(tx_order_2, &[stale2])
        .unwrap();

    // Query stale indices before different cutoffs
    let stale_before_100 = store
        .get_prune_store()
        .list_before(tx_order_1, 1000)
        .unwrap();

    let stale_before_200 = store
        .get_prune_store()
        .list_before(tx_order_2, 1000)
        .unwrap();

    let stale_before_201 = store
        .get_prune_store()
        .list_before(tx_order_2 + 1, 1000)
        .unwrap();

    println!(
        "Stale nodes by cutoff: before_100={}, before_200={}, before_201={}",
        stale_before_100.len(),
        stale_before_200.len(),
        stale_before_201.len()
    );

    // Validate monotonic behavior: more nodes should be available with higher cutoffs
    assert!(
        stale_before_201.len() >= stale_before_200.len(),
        "Higher cutoff should return at least as many nodes"
    );
    assert!(
        stale_before_200.len() >= stale_before_100.len(),
        "Higher cutoff should return at least as many nodes"
    );
    ...
    for (i, (timestamp, hash)) in stale_before_201.iter().enumerate() {
        ...
        assert_ne!(
            *timestamp,
            H256::default(),
            "Timestamp should be properly set"
        );
    }
...
async fn test_no_stale_nodes_scenario() {
    ...
    let stale_nodes = store.get_prune_store().list_before(u64::MAX, 1000).unwrap();

    println!(
        "Found {} stale nodes (should be empty or minimal)",
        stale_nodes.len()
    );
...
async fn test_all_nodes_stale_scenario() {
    ...
    let builder = ReachableBuilder::new(store.clone(), bloom.clone());
    let _scanned_size = builder.build(reachable_roots, 1).unwrap();
