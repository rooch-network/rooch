...
async fn test_bloom_saturation_false_positives() {
    ...
    let bloom = Arc::new(Mutex::new(moveos_common::bloom_filter::BloomFilter::new(1 << 8, 4))); // Small filter
    ...
    println!("Pre-loaded {} random hashes into small bloom filter", saturation_count);
    ...
    // Check if our expected nodes are already "false positive" in bloom
    {
        ...
        drop(bloom_guard);
        println!("Root hash already in pre-loaded bloom: {}", root_already_in_bloom);
    }
    ...
    println!("BuildReach scanned {} nodes with saturated bloom filter", scanned_size);
    ...
    assert!(scanned_size > 0, "BuildReach should have scanned some nodes despite bloom saturation");

    // The root should definitely be reachable since we started from it
    assert!(root_in_bloom, "Root should be reachable even with saturated bloom");
    ...
    println!("Found {} out of {} expected reachable nodes", reachable_count, expected_reachable_nodes.len());

    assert!(reachable_count >= 1, "Should find at least the root as reachable");
...
async fn test_bloom_capacity_boundaries() {
    ...
    let small_bloom = Arc::new(Mutex::new(moveos_common::bloom_filter::BloomFilter::new(1 << 4, 2))); // Very small
    ...
    println!("Small bloom filter execution time: {:?}", small_bloom_duration);

    // Test 2: Large bloom filter (high capacity)
    println!("Testing with large bloom filter...");
    let large_bloom = Arc::new(Mutex::new(moveos_common::bloom_filter::BloomFilter::new(1 << 20, 8))); // Large
    let start_time = std::time::Instant::now();
    ...
    println!("Large bloom filter execution time: {:?}", large_bloom_duration);
    ...
    assert!(ratio < 100.0, "Large bloom shouldn't be dramatically slower than small bloom");
...
async fn test_bloom_concurrent_access() {
    ...
    println!("BuildReach scanned {} nodes under concurrent simulation", scanned_size);
    ...
    println!("Found {} reachable nodes out of {}", reachable_count, all_node_hashes.len());
...
async fn test_bloom_memory_usage() {
    ...
    let small_bloom = Arc::new(Mutex::new(moveos_common::bloom_filter::BloomFilter::new(1 << 10, 4)));
    let medium_bloom = Arc::new(Mutex::new(moveos_common::bloom_filter::BloomFilter::new(1 << 16, 4)));
    let large_bloom = Arc::new(Mutex::new(moveos_common::bloom_filter::BloomFilter::new(1 << 20, 4)));
    ...
    for (size_name, bloom) in [
        ...
        println!("{} bloom: scanned {} nodes in {:?}, root reachable: {}",
                 size_name, scanned_size, duration, root_reachable);

        assert!(root_reachable, "{} bloom should have root reachable", size_name);
        assert!(scanned_size > 0, "{} bloom should have scanned nodes", size_name);
    }
