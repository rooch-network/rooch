...
impl TreeBuilder {
    ...
    pub fn inject_stale_indices(
        &self,
        nodes: Vec<H256>,
        tx_order: u64,
    ) -> Result<(), anyhow::Error> {
        // Create root entries for stale indices
        let root_entries: Vec<(H256, H256)> = nodes
            .iter()
            .map(|&node_hash| {
        ...
        self.store
            .get_prune_store()
            .write_stale_indices(tx_order, &root_entries)?;
        Ok(())
    }
...
impl BloomInspector {
    ...
    pub fn contains_all_hashes(
        ...
        expected_hashes
            .iter()
            .all(|hash| bloom_guard.contains(hash))
    }

    /// Create a test bloom filter with known hashes
    #[allow(dead_code)]
    pub fn create_test_bloom_with_hashes(
        hashes: &[H256],
    ) -> parking_lot::Mutex<moveos_common::bloom_filter::BloomFilter> {
