...
mod tests {
    ...
    impl SimpleTestDatabase {
        ...
        /// Create test database with startup info in the config store
        pub fn with_startup_info() -> Result<Self> {
            let mut db = Self::new()?;

            // Use MoveOSStore mock to create config store
            let (moveos_store, _temp_dir) = moveos_store::MoveOSStore::mock_moveos_store()?;

            // Create startup info (using a simple constructor)
            // Let's try to create StartupInfo with minimal required fields
            let startup_info = StartupInfo {
                chain_id: 0, // Dev chain
                state_root: db.state_root,
                timestamp: 1640995200000, // 2022-01-01 00:00:00 UTC
            };

            // Store startup info using the config store
            moveos_store.config_store.set_startup_info(&startup_info)?;

            info!(
                "Created test database with startup info and state_root: {}",
                db.state_root
            );
    ...
    fn test_gc_with_simple_database() -> Result<()> {
        ...
        // Create test database with startup info
        let _test_db = SimpleTestDatabase::with_startup_info()?;
        info!(
            "Created test database with state_root: {}",
            test_db.state_root()
        );
    ...
    fn test_gc_error_handling_with_simple_database() -> Result<()> {
        ...
        let _test_db = SimpleTestDatabase::new()?; // Without startup info
        let (moveos_store, temp_dir) = moveos_store::MoveOSStore::mock_moveos_store()?;
        let moveos_store = Arc::new(moveos_store);
