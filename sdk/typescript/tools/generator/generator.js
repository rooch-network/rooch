...
export class Generator {
    ...
    async execute(openRPCDoc) {
        ...
        await this.renderClient(openRPCDoc.methods)
    }
    ...
    async renderClient(methods) {
        const templatePath = path.join(__dirname, 'template/client.ts.ejs');  // 模板文件路径
        const templateStr = fs.readFileSync(templatePath, 'utf-8');
        const renderResult = ejs.render(templateStr, { methods, getType });
        const prettyRenderResult = await prettier.format(renderResult, { parser: "typescript" });

        const note = "// This file was generated by `yarn gen:client`. Please, do not modify it.";
        const resultWithNote = `${note}\n${prettyRenderResult}\n${note}`;

        fs.writeFileSync(path.join(this.outputDir, 'client.ts'), resultWithNote);
    }
}
