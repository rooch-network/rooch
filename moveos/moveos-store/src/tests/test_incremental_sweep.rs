...
async fn test_incremental_sweep_via_handle_tx_output() {
    ...
    let (_tx_output_v1, _exec_info_v1) = store
        .handle_tx_output(tx_hash_v1, output_v1)
        .unwrap();
    
    let state_root_v1 = changeset_v1.state_root;
    println!("✅ Transaction v1 applied with state_root: {}", state_root_v1);
    ...
    println!("   Nodes with refcount > 0: {}/{}", nodes_with_refcount, nodes_v1.len());
    assert!(nodes_with_refcount > 0, "Should have nodes with refcount > 0");
    ...
    let (_tx_output_v2, _exec_info_v2) = store
        .handle_tx_output(tx_hash_v2, output_v2)
        .unwrap();
    
    let state_root_v2 = changeset_v2.state_root;
    println!("✅ Transaction v2 applied with state_root: {}", state_root_v2);
    ...
    assert!(!stale_list.is_empty(), "Should have stale indices after second transaction");
    ...
    for (_stale_root, node_hash) in &stale_list {
        ...
        if refcount == 0 {
            ...
            let exists = store.get_state_node_store().get(node_hash).unwrap().is_some();
            assert!(exists, "Node {} should exist before sweep", node_hash);
        }
    }
    
    println!("   Nodes with refcount == 0: {}", nodes_with_zero_refcount.len());
    assert!(!nodes_with_zero_refcount.is_empty(), 
    ...
    for (stale_root, node_hash) in stale_list {
        ...
        if refcount == 0 {
            // Delete the node
            store.get_state_node_store().delete_nodes(vec![node_hash]).unwrap();
            // Remove metadata
            store.prune_store.remove_stale_indice((stale_root, node_hash)).unwrap();
...
async fn test_refcount_prevents_premature_deletion() {
    ...
    // Create a node
    store.get_state_node_store().put(node_hash, node_data.clone()).unwrap();
    ...
    store.prune_store.write_stale_indices(&[(root, node_hash)]).unwrap();
    
    // Refcount should be 2 now (was 3, decremented to 2)
    let refcount = store.prune_store.get_node_refcount(node_hash).unwrap();
    assert_eq!(refcount, 2, "Node should have refcount == 2 after one write_stale_indices");
    ...
    for (_stale_root, stale_node_hash) in stale_list {
        if stale_node_hash == node_hash {
            let refcount = store.prune_store.get_node_refcount(stale_node_hash).unwrap();
            assert!(refcount > 0, "Node should have refcount > 0");
            
            // Should NOT delete
            let exists = store.get_state_node_store().get(&stale_node_hash).unwrap().is_some();
            assert!(exists, "Node {} with refcount={} should NOT be deleted", 
