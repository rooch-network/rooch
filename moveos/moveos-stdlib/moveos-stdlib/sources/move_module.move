...
module moveos_std::move_module {
    ...
    public fun replace_string_constant(
        modules: vector<MoveModule>,
    ...
    #[test(account=@0x42)]
    fun test_replace_bytes_constant(account: &signer) {
        let addr = signer::address_of(account);
        let ctx = context::new_test_context(addr);
    ...
    fun test_replace_string_constant(account: &signer) {
        ...
        vector::push_back(&mut new_strings, string::utf8(b"My Fixed Supply Coin"));
        vector::push_back(&mut new_strings, string::utf8(b"MFSC"));
        let old_strings = vector::empty<String>();
        vector::push_back(&mut old_strings, string::utf8(b"Fixed Supply Coin"));
        vector::push_back(&mut old_strings, string::utf8(b"FSC"));

        let new_modules = Self::replace_string_constant(modules, old_strings, new_strings);

        // compare the remapped modules bytes
        let modified_bytes = vector::borrow(&new_modules, 0).byte_codes;
        assert!(std::compare::cmp_bcs_bytes(&modified_bytes, &ref_bytes) == 0u8, 1);
    ...
    fun test_module_template(account: &signer) {
        ...
        // The following is the bytes the compiled module: fixed_supply_coin_template.move
        ...
        let modules = Self::replace_string_constant(modules, old_strings, new_strings);
        ...
        let modules_bytes = vector::singleton(vector::borrow(&modules, 0).byte_codes);

        let new_supply = vector::singleton(210_000_000_000u256);
        let old_supply = vector::singleton(123_321_123_456u256);
        let modules_bytes = Self::replace_u256_constant(modules_bytes, old_supply, new_supply);

        let new_decimal = vector::singleton(1u8);
        let old_decimal = vector::singleton(222u8);
        let modules_bytes = Self::replace_u8_constant(modules_bytes, old_decimal, new_decimal);

        let module_bytes = vector::borrow(&modules_bytes, 0);
        // compare the remapped modules bytes
