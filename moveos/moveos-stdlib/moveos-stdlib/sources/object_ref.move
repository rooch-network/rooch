...
module moveos_std::object_ref {
    ...
    public(friend) fun new_internal<T: key>(object: &mut ObjectEntity<T>) : Object<T> {
        Object {
            id: object::id(object),
        }
    }

    public(friend) fun as_ref<T: key>(object: &ObjectEntity<T>) : &Object<T>{
        as_ref_inner<Object<T>>(object::id(object))
    }
    public(friend) fun as_mut_ref<T: key>(object: &mut ObjectEntity<T>) : &mut Object<T>{
        assert!(!object::is_frozen(object), error::permission_denied(ErrorObjectFrozen));
        as_mut_ref_inner<Object<T>>(object::id(object))
    }
