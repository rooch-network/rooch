...
module btc_holder_coin::holder_coin {
    ...
    /// Hold the TreasuryCapHolder object
    struct TreasuryHolder has key {
        treasury_cap: Object<TreasuryCap<HDC>>,
    }
    ...
    fun init() {
        let treasury_cap = coin::register_extend_v2<HDC>(
            string::utf8(b"BTC Holder Coin"),
            string::utf8(b"HDC"),
            option::none(),
            DECIMALS,
        );
        let treasury_holder_obj = object::new_named_object(TreasuryHolder { treasury_cap: treasury_cap });
        // Make the coin info holder object to shared, so anyone can get mutable CoinInfoHolder object
        object::to_shared(treasury_holder_obj);
    }
    ...
    public fun do_claim(treasury_holder_obj: &mut Object<TreasuryHolder>, utxo_obj: &mut Object<UTXO>): Coin<HDC> {
        ...
        let treasury_holder = object::borrow_mut(treasury_holder_obj);
        let mint_amount = (((now - stake_info.last_claim_time) * utxo_value) as u256);
        let coin = coin::mint_extend_by_cap<HDC>(&mut treasury_holder.treasury_cap, mint_amount);
    ...
    public entry fun claim(treasury_holder_obj: &mut Object<TreasuryHolder>, utxo: &mut Object<UTXO>) {
        let coin = do_claim(treasury_holder_obj, utxo);
    ...
    fun test_stake_claim() {
        ...
        let treasury_holder_obj_id = object::named_object_id<TreasuryHolder>();
        let treasury_holder_obj = object::borrow_mut_object_shared<TreasuryHolder>(treasury_holder_obj_id);
        let hdc_coin = do_claim(treasury_holder_obj, &mut utxo);
