...
module rooch_framework::did_integration_test {
    ...
    //     let initial_public_key = did_test_common::generate_test_secp256k1_multibase_key();
    ...
    //         assert!(did::test_verification_method_exists(did_document, &fragment), error_code);
    //         j = j + 1;
    //     };
    // }
    ...
    //     let vm_type = string::utf8(b"Ed25519VerificationKey2020");
    //     let vm_key = did_test_common::generate_test_ed25519_multibase_key();
    //     let vm_relationships = vector[0u8, 2u8]; // authentication, capability_invocation
    ...
    //     // Test 4: Permission Integration - Switch to new verification method
    ...
    //     did::add_service_entry(&test_signer, service2_fragment, service2_type, service2_endpoint);

    //     // Test 5: Query Integration - Verify all components
    //     let final_did_document = did::get_did_document_for_testing(test_address);
    ...
    //     assert!(did::has_verification_relationship_in_doc(final_did_document, &vm_fragment, 0), 15206); // authentication
    ...
    //     let final_relationships = vector[0u8, 1u8]; // authentication, assertion_method

    //     did::add_verification_method_entry(&test_signer, final_fragment, final_type, final_key, final_relationships);

    //     // Scenario 3: Verify DID document integrity
    //     let did_document = did::get_did_document_for_testing(test_address);
    //     assert!(did::test_verification_method_exists(did_document, &final_fragment), 15301);
    //     assert!(did::test_verification_method_exists(did_document, &string::utf8(b"account-key")), 15302); // Original still exists
    //     assert!(did::has_verification_relationship_in_doc(did_document, &final_fragment, 0), 15303); // authentication
    //     assert!(did::has_verification_relationship_in_doc(did_document, &final_fragment, 1), 15304); // assertion_method
