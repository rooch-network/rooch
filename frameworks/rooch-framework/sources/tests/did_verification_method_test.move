...
module rooch_framework::did_verification_method_test {
    ...
    #[test]
    /// Test adding RS256 verification method successfully
    fun test_add_verification_method_rs256() {
        // Use proper setup to get creator info and DID object ID
        let (_creator_signer, _creator_address, _creator_public_key_multibase, did_object_id) = did_test_common::setup_did_test_with_creation();
        ...
        // Add RS256 verification method
        let fragment = string::utf8(b"rs256-key");
        let method_type = string::utf8(b"Rs256VerificationKey2018");
        let rs256_key = did_test_common::generate_test_rs256_multibase();
        let relationships = vector[3u8, 4u8]; // assertion_method, capability_invocation
        ...
        let did_document_after = did::get_did_document_by_address(did_address);
        assert!(did::test_verification_method_exists(did_document_after, &fragment), 15001);
        assert!(did::has_verification_relationship_in_doc(did_document_after, &fragment, 3), 15002); // assertion_method
        assert!(did::has_verification_relationship_in_doc(did_document_after, &fragment, 4), 15003); // capability_invocation
    }

    #[test]
    #[expected_failure(abort_code = did::ErrorVerificationMethodAlreadyExists, location = rooch_framework::did)] // ErrorVerificationMethodAlreadyExists
    /// Test adding duplicate RS256 verification method fails
    fun test_add_duplicate_rs256_verification_method() {
        // Use proper setup to get creator info and DID object ID
        let (_creator_signer, _creator_address, _creator_public_key_multibase, did_object_id) = did_test_common::setup_did_test_with_creation();
        ...
        // Add first RS256 verification method
        let fragment = string::utf8(b"rs256-key");
        let method_type = string::utf8(b"Rs256VerificationKey2018");
        let rs256_key = did_test_common::generate_test_rs256_multibase();
        let relationships = vector[3u8]; // assertion_metho
    ...
    #[test]
    /// Test removing RS256 verification method
    fun test_remove_rs256_verification_method() {
        // Use proper setup to get creator info and DID object ID
        let (_creator_signer, _creator_address, _creator_public_key_multibase, did_object_id) = did_test_common::setup_did_test_with_creation();
    ...
    #[test]
    /// Test adding and removing RS256 verification method from relationships
    fun test_rs256_verification_relationships() {
        // Use proper setup to get creator info and DID object ID
        let (_creator_signer, _creator_address, _creator_public_key_multibase, did_object_id) = did_test_common::setup_did_test_with_creation();
        ...
        // Add RS256 verification method with initial relationships
        let fragment = string::utf8(b"rs256-key");
        let method_type = string::utf8(b"Rs256VerificationKey2018");
        let rs256_key = did_test_common::generate_test_rs256_multibase();
        let relationships = vector[3u8]; // assertion_method only
        ...
        // Verify initial relationships
        let did_document_check = did::get_did_document_by_address(did_address);
        assert!(did::has_verification_relationship_in_doc(did_document_check, &fragment, 3), 17001); // assertion_method
        assert!(!did::has_verification_relationship_in_doc(did_document_check, &fragment, 4), 17002); // capability_invocation

        // Add capability_invocation relationship
        did::add_to_verification_relationship_entry(&did_signer, fragment, 4u8);

        // Verify new relationship was added
        let did_document_after_add = did::get_did_document_by_address(did_address);
        assert!(did::has_verification_relationship_in_doc(did_document_after_add, &fragment, 4), 17003); // capability_invocation

        // Remove capability_invocation relationship
        did::remove_from_verification_relationship_entry(&did_signer, fragment, 4u8);

        // Verify relationship was removed
        let did_document_after_remove = did::get_did_document_by_address(did_address);
        assert!(!did::has_verification_relationship_in_doc(did_document_after_remove, &fragment, 4), 17004); // capability_invocation
    }
}
