...
module rooch_framework::did_error_test {
    ...
    // #[test]
    // #[expected_failure(abort_code = 27, location = rooch_framework::did)] // ErrorSignerNotDIDAccount
    ...
    //     let correct_signer = did_test_common::create_test_account_and_signer();
    //     let wrong_signer = moveos_std::account::create_signer_for_testing(@0x999);
    ...
    //     // Create first DID
    //     let signer1 = did_test_common::create_test_account_and_signer();
    //     let address1 = signer::address_of(&signer1);
    //     let public_key1 = did_test_common::generate_test_secp256k1_multibase_key();
    //     did::create_did_object_for_self_entry_test_only(&signer1, public_key1);

    //     // Create second DID  
    //     let signer2 = moveos_std::account::create_signer_for_testing(@0x999);
    ...
    // #[test]
    // #[expected_failure(abort_code = 20, location = moveos_std::multibase)] // ErrorInvalidPublicKeyMultibaseFormat
    // /// Test error for invalid multibase public key format
    // fun test_error_invalid_public_key_multibase_format() {
    //     use moveos_std::multibase;
        
    //     // Try to decode invalid multibase key
    //     let invalid_multibase_key = string::utf8(b"invalid_multibase_key");
    //     let _ = multibase::decode_ed25519_key(&invalid_multibase_key);
    // }
    ...
    //     did::add_verification_method_entry(&test_signer, fragment, method_type, test_key1, relationships);

    //     // Verify method was added
    //     let did_document = did::get_did_document_for_testing(test_address);
    //     assert!(did::test_verification_method_exists(did_document, &fragment), 13401);
    ...
    //     let did_document = did::get_did_document_for_testing(signer::address_of(&test_signer));
    //     assert!(did::test_service_exists(did_document, &fragment), 13601);
    // }
    ...
    //     // Verify method was added
    //     let did_document = did::get_did_document_for_testing(test_address);
    //     assert!(did::test_verification_method_exists(did_document, &fragment), 13703);
    // }
