...
module rooch_framework::gas_coin {
    ...
    use rooch_framework::coin::{Self, Coin, CoinInfo, TreasuryCap};
    ...
    fun borrow_mut_treasury_cap() : &mut Object<TreasuryCap<RGas>> {
        let signer = signer::module_signer<RGas>();
        let treasury_cap_id = coin::treasury_cap_id<RGas>();
        object::borrow_mut_object<TreasuryCap<RGas>>(&signer, treasury_cap_id)
    }

    fun mint(amount: u256): Coin<RGas> {
        let treasury_cap = borrow_mut_treasury_cap();
        coin::mint_extend_by_cap<RGas>(treasury_cap, amount)
    }
    ...
    public fun burn(coin: Coin<RGas>) {
        let treasury_cap = borrow_mut_treasury_cap(); 
        coin::burn_extend_by_cap<RGas>(treasury_cap, coin);
    }
    ...
    public(friend) fun genesis_init(_genesis_account: &signer){
        ...
        let treasury_cap_obj = coin::register_extend_v2<RGas>(
            string::utf8(b"Rooch Gas Coin"),
            string::utf8(b"RGAS"),
            option::some(string::utf8(rgas_image)),
            DECIMALS, // decimals
        );
        object::transfer(treasury_cap_obj, @rooch_framework);
    }

    public(friend) fun upgrade_v19_init(){
        let framework_signer = signer::module_signer<RGas>();
        let coin_info_obj = object::take_object<CoinInfo<RGas>>(&framework_signer, object::named_object_id<CoinInfo<RGas>>());
        let treasury_cap_obj = coin::exchange_cap<RGas>(coin_info_obj);
        object::transfer(treasury_cap_obj, @rooch_framework);
    }
}
