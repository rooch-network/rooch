...
module moveos_std::table {
    ...
    public fun add<K: copy + drop, V:store>(table: &mut Table<K, V>, key: K, val: V) {
    ...
    public fun borrow_with_default<K: copy + drop, V:store>(table: &Table<K, V>, key: K, default: &V): &V {
    ...
    public fun borrow_mut_with_default<K: copy + drop, V: store + drop>(table: &mut Table<K, V>, key: K, default: V): &mut V {
        object::borrow_mut_field_with_default(&mut table.handle, key, default)
    }

    /// Insert the pair (`key`, `value`) if there is no entry for `key`.
    /// update the value of the entry for `key` to `value` otherwise
    public fun upsert<K: copy + drop, V: store + drop>(table: &mut Table<K, V>, key: K, value: V) {
        object::upsert_field(&mut table.handle, key, value)
    }

    /// Remove from `table` and return the value which `key` maps to.
    /// Aborts if there is no entry for `key`.
    public fun remove<K: copy + drop, V:store>(table: &mut Table<K, V>, key: K): V {
        object::remove_field(&mut table.handle, key)
    }

    /// Returns true if `table` contains an entry for `key`.
    public fun contains<K: copy + drop, V:store>(table: &Table<K, V>, key: K): bool {
        object::contains_field(&table.handle, key)
    }

    /// Destroy a table. Aborts if the table is not empty.
    public fun destroy_empty<K: copy + drop, V:store>(table: Table<K, V>) {
        let Table { handle } = table;
        let TablePlaceholder{_placeholder:_} = object::remove(handle);
    }

    /// Returns the size of the table, the number of key-value pairs
    public fun length<K: copy + drop, V:store>(table: &Table<K, V>): u64 {
        object::field_size(&table.handle)
    }

    /// Returns true iff the table is empty (if `length` returns `0`)
    public fun is_empty<K: copy + drop, V:store>(table: &Table<K, V>): bool {
    ...
    public fun drop_unchecked<K: copy + drop, V:store>(table: Table<K, V>) {
        let Table { handle } = table;
        let TablePlaceholder{_placeholder:_} = object::remove_unchecked(handle);
    }
    ...
    public fun handle<K: copy + drop, V:store>(table: &Table<K, V>): ObjectID {
