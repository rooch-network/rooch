...
module moveos_std::object {
    ...
    // Ensure the objectID generated by named_object_id_with_type is the same as the one generated by named_object_id
    public fun named_object_id_with_type<T>(type: string::String): ObjectID {
        address_to_object_id(
            address::from_bytes(
                hash::sha3_256(
                    std::string::into_bytes(type_info::type_name<T>())
    ...
    public fun account_named_object_id<T>(account: address): ObjectID {
        ...
        // account_named_object_id_with_type(account, type)
    }

    public fun account_named_object_id_with_type<T>(account: address, type: string::String): ObjectID {
        let bytes = bcs::to_bytes(&account);
        vector::append(&mut bytes, *std::string::bytes(&type_info::type_name<T>()));
        vector::append(&mut bytes, *std::string::bytes(&type));
        address_to_object_id(
            address::from_bytes(
                hash::sha3_256(bytes)
            )
