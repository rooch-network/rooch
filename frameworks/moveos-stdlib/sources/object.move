...
module moveos_std::object {
    ...
    public fun custom_object_id<ID: store + copy + drop, T: key>(id: ID): ObjectID {
        address_to_object_id(derive_global_object_key<T, ID>(id))
    }

    public fun custom_child_object_id<ID: store + copy + drop>(parent_id: ObjectID, id: ID): ObjectID {
        let child = derive_field_key(id);
    ...
    #[private_generics(T, Value)]
    /// Add a object field to the object. return the child object
    /// The parent object must be a shared object
    public fun add_object_field<T: key, Value: key>(obj: &mut Object<T>, v: Value): Object<Value> {
        ...
        native_add_field<Value>(obj.id, child_key, v)
    }
    ...
    public fun add_object_field_with_id<T: key, ID: store + copy + drop, Value: key>(obj: &mut Object<T>, id: ID, v: Value): Object<Value> {
        // Only shared object can add child object
        assert!(is_shared(obj), ErrorObjectNotShared);
        // Currently, the child object level is limited to 2
        assert!(vector::length(&obj.id.path) < 2, ErrorChildObjectTooDeep);
        let child_key = derive_field_key(id);
        native_add_field<Value>(obj.id, child_key, v)
    }
    ...
    /// Derive a global object key from the `id` and the type name of `T`
    /// hash(id || T_type_tag)
    ...
    fun derive_global_object_key<T: key, ID: store + copy + drop>(id: ID): address {
