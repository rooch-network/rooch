...
module rooch_nursery::ton_validator {
    ...
    public fun validate_signature(ton_address: &TonAddress, proof: &TonProof, tx_hash: vector<u8>) {
        assert!(ton_proof::verify_proof(ton_address, proof), auth_validator::error_validate_invalid_authenticator());
    ...
    public fun validate(authenticator_payload: vector<u8>) {
        let proof = ton_proof::decode_proof(authenticator_payload);
        ...
        validate_signature(&sender_ton_addr, &proof, tx_hash);
    }
}
