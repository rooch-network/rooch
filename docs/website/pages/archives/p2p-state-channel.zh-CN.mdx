...
Also, Rooch supports smart contract in state channel through the feature of Move, simplifying the design of game and DApp protocol on P2P networks.
...
3. One participant frauds, and the other participant can use [fraud-proofs] (./fraud-proof.zh-CN.mdx) to initiate an arbitration request at the Rooch execution layer for settlement by the arbitration contract.
