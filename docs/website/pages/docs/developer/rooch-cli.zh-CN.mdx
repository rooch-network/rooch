...
使用 `rooch account help <subcommand>` 查看每个子命令的更多信息。

#### Account Create

使用 `rooch account create` 子命令在 Rooch 网络上创建账户。

Rooch CLI 账户创建子命令支持以下选项：
...
如果不输入 `--config-dir` 选项，账户将在默认的本地文件路径下创建，当前路径为 `~/.rooch/rooch_config/`。

值得注意的是，要创建账户，配置文件 `rooch.yaml` 必须存在。`rooch.yaml` 文件可以通过 `rooch init` 命令创建。

以下是创建账户时使用和不使用选项 `--config-dir` 的示例：
...
在第 1-2 行，创建子命令将生成一个新的密钥对，并根据方案 ed25519 创建一个地址。

在第 3 行，一个秘密恢复短语或助记符短语被赋予一个地址，用于恢复地址。

第 4-57 行，向 Rooch 区块链 RPC 服务器发送事务，并在此返回相应的响应。
...
还可以为账户创建命令指定一个选项 `--config-dir`，以指定配置目录。
...
在 `rooch init` 命令后使用 `rooch account create` 创建的账户有两个可用账户。

使用 `rooch account switch` 子命令可在两个账户间切换。

Rooch CLI Account Switch 子命令支持以下选项：
...
使用 `rooch account list` 子命令检查账户状态。

Rooch CLI Account List 子命令支持以下选项：
...
使用 `rooch account import` 子命令从助记符短语中导入账户。

Rooch CLI Account Import 子命令支持以下选项：
...
使用 `--mnemonic-phrase` 或 `-m` 标志指定一个助记符短语来导入账户。账户地址将从助记符短语生成一个 ed25519 地址，并生成一个 ed25519 密钥对。
...
使用 `rooch account update` 子命令用来自助记符短语和方案的新密钥对更新地址。

Rooch CLI Account Update 子命令支持以下选项：
...
使用 `--address` 或 `-a`、`-mnemonic-phrase` 或 `-m`、`-scheme` 或 `-s` 标志组合更新帐户地址。
...
使用 `rooch account nullify` 子命令从账户地址中的方案中删除现有密钥对。

Rooch CLI Account Nullify 子命令支持以下选项：
...
使用 `--address` 或 `-a`、`--scheme` 或 `-s` 标志组合可使账户地址中的键对无效。

同样，`<ADDRESS>` 应是账户中现有的地址，`<CRYPTO_SCHEMES>` 应在可能的值中选择。

下面是一个示例，用于使新添加的 ecdsa 方案的密钥对无效：
...
命令行输出显示，通过向 Rooch 区块链 RPC 服务器发送交易，ecdsa 方案已从账户地址中删除，ecdsa 方案的验证密钥也已从发送者的账户存储中删除。
