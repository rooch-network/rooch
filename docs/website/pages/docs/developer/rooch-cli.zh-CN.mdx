...
了解如何设置、配置和使用 Rooch 命令行界面（CLI）。您可以通过 Rooch CLI 来试验 Rooch 特性。

## Set up

在安装 Rooch 时，将安装 Rooch CLI。有关先决条件和安装说明，请参阅[安装](./installation.zh-CN.mdx)主题。
...
如果不输入 `--config-dir` 选项来指定路径，账户将在默认的本地文件路径下创建，当前路径为 `~/.rooch/rooch_config/`。
...
在第 3 行，一个密钥恢复短语或助记符短语被赋予一个地址，用于恢复地址。
