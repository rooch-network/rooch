...
Context 包含两个字段，一个是 `TxContext` 包含当前交易相关的信息，另外一个是 `StorageContext`，对应第一层的状态树。
...
开发者可以在 `entry` 方法中定义 Context 参数，MoveVM 会自动填充该参数。
...
当前 RawTable 中的方法都是 `friend`，开发者无法直接操作，需要通过 RawTable 上层封装的数据结构进行操作。比如 `Table<K,V>`，`TypeTable`，`StorageContext` 。
...
Rooch Object 也是 Move 中的 [Hot Potato](https://examples.sui.io/patterns/hot-potato.html) 模式的一个使用案例。Object 不具有任何 `ability`，所以它不可以被 `drop`，`copy`，`store`，创建之后只能被 StorageContext 的接口处理。
...
StorageContext 提供了 Object 全局存储相关的接口，它基于 RawTable 实现，定义如下：
...
`Context` 为开发者提供了操作 `Object` 的接口。其基于 `StorageContext` 实现，具体定义如下：
...
同时，这种方式还解决了一个 Move 的引用难题，开发者无法在自己的方法中返回通过 `borrow_global<T>` 指令借用的引用，因为这个引用是凭空创造的，不能作为方法的返回值。而如果通过 `Context` 借用，则可以达到这个目标，该引用的生命周期和 `StorageContext` 的生命周期绑定。
