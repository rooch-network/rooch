...
Rooch 中的 Object 采用一种箱子（Box）模式的 Object。创建一个 Object 相当于在 Object Storage 空间中创建了一个箱子(`ObjectEntity<T>`)，可以将类型 `T` 的实例封装在箱子中，而返回的 `Object<T>` 是这个箱子的钥匙，`ObjectID` 是这个箱子的地址。
