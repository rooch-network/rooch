...
`#[private_generics(T)]` is a function annotation, which ensures that the function's generic parameter `T` can only be a type defined in the current module of the caller. The security model follows similar constraints to the Move storage instruction, but opens this capability to developers. This feature is the basic condition for the following features.
