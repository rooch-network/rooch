...
Rooch currently has three built-in standard libraries, namely `MoveStdlib`, `MoveosStdlib` and `RoochFramework`. For details, see the [built-in libraries](./library).
...
In addition to `Account Resource，` Rooch also provides Object Storage, For detailed information about Rooch Object, see [Rooch Object](./object).

Regarding the design of storage abstraction, you can refer to [Storage Abstraction](../dive-into-rooch/storage-abstraction).
