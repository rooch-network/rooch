...
同样，我们定义 `init` 来调用创建计数器的逻辑函数，可以看到，我们分别调用了两个函数：`create_shared` 和 `create_user`。这个例子中，我们演示 Rooch 的对象存储模型中的两种对象的区别和和作用。

<++>

我们将初始化计数器的逻辑从 `init` 函数中剥离出来，此时的 `init` 函数只是简单调用创建计数器的函数 `create_counter`，并打印计数器对象的 ID。

在 `create_counter` 函数中，首先初始化计数器，获取签名交易的地址，然后创建计数器对象和计数器对象的 ID，再将计数器对象转移到发起交易的地址，最后返回对象 ID。

递增计数值的 `increase` 函数中，我们调用 `borrow_mut` 获取计数器对象的可变引用，当我们拿到计数器对象之后就可以对它的内部值进行修改了，如同上边的操作一样，我们通过 `+1` 来实现计数值的递增。
...
```shell
0xf049ae1d92f970ff1b9c397a49bb5f7c7668bfd5cf5b73035c183b574a7daabb
```

2. 使用 `rooch object --id ID` 来查看计数器对象的内容：
