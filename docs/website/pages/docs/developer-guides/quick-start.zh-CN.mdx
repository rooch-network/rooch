...
我们将初始化计数器的逻辑从 `init` 函数中剥离出来，此时的 `init` 函数只是简单调用创建计数器的函数 `create_counter`。

递增计数值的 `increase` 函数中，我们首先获取调用者的地址，并存放在 `owner_addr` 变量中，并调用 `borrow_mut_resource` 获取计数器对象的可变引用，当我们拿到计数器对象之后就可以对它的内部值进行修改了，如同上边的操作一样，我们通过 `+1` 来实现计数值的递增。

此外，我们另外提供了一个 `query` 函数用来获取计数器的计数值，我们可以使用 `rooch move view` 命令来调用这个函数并检查计数值的变化。
...
2. 调用 `query` 函数，检查计数器的值，调用语法是：`rooch move view --function 模块地址::模块名::函数名 --args 参数列表`，这个函数需要显式接受一个地址参数，需要使用 `address:值` 来标识。
...
可以看到，计数器的值已经使用 `0` 进行初始化了，我们也可以使用 `rooch resource` 命令来检查一下：
