...
Object in Rooch is designed as a Box style Object. When an Object is created, it is essentially like creating a box (`ObjectEntity<T>`) in global Object Storage, encapsulating an instance of type `T` inside it. The returned `Object<T>` serves as the key to this box, while `ObjectID` is the address of the box.

## Definitions of ObjectEntity and Object
...
`ObjectEntity` has no `ability` and can only be accessed by the underlying storage interface. Developers can access the encapsulated data in `ObjectEntity<T>` through `Object<T>`. The lifecycles of the two are identical, and developers only need to operate the API related to `Object<T>`, without considering `ObjectEntity<T>`.
...
7. [StorageAbstraction](../dive-into-rooch/storage-abstraction)
