...
Rooch 中有三种实现交易包含，公开性和强制性逐步提升：

1. 直接请求 Sequencer
2. 通过 L1 （任意结算链）的交易队列提交
3. 通过 L1 （仲裁链）的交易队列提交

*在 Rooch 中 L1 分两种角色：结算链，仲裁链。结算链可实现与 Rooch 的信息传递，仲裁链可在结算链的基础之上提供仲裁能力。*

### 直接请求 Sequencer

用户可以直接请求 Sequencer 对交易进行包含，Sequencer 会将交易添加到 Accumulator 中，并立刻返回交易的全局顺序。
这种包含是公开且可仲裁的，但 Sequencer 可以不响应用户请求从而实现交易审查攻击。当用户担心交易审查时，用户可通过第三种方式提交。

### 通过 L1 的交易队列提交

用户可以将交易提交到 L1 的交易队列中，等待 Sequencer 抓取。这种包含是公开但不是所有情况都可仲裁。这类交易通常表现为由 L1 向 L2 发送消息，如存款交易。
对于非仲裁链，当用户担心交易审查时，用户可通过第三种方式提交。

### 通过 L1 （仲裁链）的交易队列提交

通过 L1 （仲裁链）的交易队列提交，等待 Sequencer 获取，并且可通过 L1 的仲裁合约进行强制包含仲裁。仲裁机制如下：

1. 用户将交易提交到 L1 的仲裁链的交易队列合约中。
2. Sequencer 承诺进入交易队列合约后将在一定时间内（t）获取交易。
3. 如果 Sequencer 在 t 时间内没有获取交易，任意用户可以通过仲裁合约发起仲裁要求 Sequencer 提供交易包含在某个 tx list 的累加器证明。
4. 该累加器证明可在仲裁链上验证，如果验证失败（非法证明或 tx 所在 tx list 的创建时间大于 tx 创建时间 + t）或者 Sequencer 无法在规定时间（t2）内提供证明，Sequencer 将被罚没一部分押金，仲裁发起者可获得相应的奖金。
5. 若 t3 时间后 Sequencer 仍未收入交易，任意用户可发起冻结仲裁，Sequencer 将被冻结一段时间（t4），在这段时间内作恶的 Sequencer 无法向 L1 提交状态更新并被罚没所有押金[^1]。 
...
由于上述仲裁机制的存在，交易审查必被捕获且面临巨额惩罚，Sequencer 将不得不提供可靠的服务。
...
[^1]: 该惩罚机制的目的是防止 Sequencer 作恶，但也会导致 Sequencer 无法提供服务。
