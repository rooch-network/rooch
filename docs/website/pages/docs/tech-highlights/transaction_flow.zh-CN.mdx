...
这种包含是可仲裁的（通过仲裁链），但 Sequencer 可以不响应用户请求从而实现交易审查攻击。当用户担心交易审查时，可通过第三种方式提交交易。

### 2. 通过 L1 提交

用户可以通过 L1 发起 L2 调用，等待 Sequencer 抓取并执行。
该方式使得 Sequencer 无法隐蔽的实现审查交易，因为任何人都可以很容易发现 L1 上的合约调用与相关事件，以及 L2 上是否包含该交易的事实。
因此，只要我们把这个链下验证过程重现在链上，那么便可以实现对作恶 Sequencer 的惩罚。这需要我们在 Rooch 上用合约实现各 L1 的轻节点。
...
4. Sequencer 承诺 L2 state commitment 提交的最大间隔为 t3。 t3 << t2，以确保 L2 上的 L1 区块信息更新 tx 的执行结果可用于仲裁，同时也意味着该结果可被 verifier 验证和挑战以确保区块信息的正确性。L1 合约会检查每次提交的间隔。
5. 在任意结算链上的任意用户发现 Sequencer 在 t0+t1 时间内未完成交易，任意用户可在链下生成 L1ToL2 事件的存在证明以及区块头向仲裁链申请包含仲裁。当仲裁合约收到仲裁请求，将要求 Sequencer 在 t4 时间内提供如下证明否则被惩罚：
   1. 若用户提供的区块头显示区块时间不满足挑战，则退出。
   2. 提供该 L1ToL2 事件所在区块高度的区块头，或指出用户证据中的区块头不存在或非法（其结果会在合约中保存一个挑战期）。
   3. 提供 L2 验证该区块头以及最新区块头的 L2 tx 及其累加器证明 （会在合约中保存一个挑战期），以供 verifier 验证结果。
   4. verifier 持续验证上述挑战合约中的 L2 tx 是否和 Sequencer 提供给仲裁合约的 L1 区块头一致，若不一致可发起欺诈证明。此步骤不在本仲裁合约内，但为本合约的正确执行提供了保障，它属于 verifier 的验证工作的一部份。
   5. 若最新区块头不满足 t2，惩罚 Sequencer，挑战终止。用户后续可每隔 t1 时间持续性发起同一交易的包含仲裁，Sequencer 的不作为将面临逐步加重的惩罚[^1]。
   6. 若用户提供的证明通过仲裁链的 merkle 树验证，且 Sequencer 提供的收据树根与用户一致则挑战继续。否则终止。
   7. 至此，L2 和用户均认为 L1ToL2 事件已经产生，接下来验证 L2 在规定时间内包含了该事件。
   8. 提供该事件对应的 L2 tx 的累加器证明，指出该 tx 所在的 tx list root 提交到仲裁链的时间。
   9. 上述累加器和时间证明可通过 L2 向仲裁链提交的状态更新得以验证。
   10. 若验证失败，惩罚 Sequencer。

* t1：24 小时
* t2：8 小时
* t4: 2 小时

实现多链交易包含的关键打破“实现交易包含需要 L2 轻节点验证，L2 轻节点的实现需要交易包含“的循环证明。通过引入轻节点时效性的机制，建立了循环的退出条件使得仲裁功能顺利进行。
