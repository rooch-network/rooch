...
这种包含是可仲裁的，但 Sequencer 可以不响应用户请求从而实现交易审查攻击。当用户担心交易审查时，可通过第三种方式提交交易。

### 2. 通过 L1 提交

用户可以通过存款交易（任何由 L1 触发的 L2 交易均成为存款交易）在 L1 上产生存款事件，等待 Sequencer 抓取。
该方式使得 Sequencer 无法隐蔽的实现审查交易，因为任何人都可以随时验证 L1 上的合约调用与存款事件，但 L2 上交易未被包含的事实。
但对于非仲裁链，我们无法直接惩罚 Sequencer.当用户担心交易审查时，可通过第三种方式提交交易。

### 3. 通过 L1 （仲裁链）的提交

用户可以通过存款交易（任何由 L1 触发的 L2 交易均成为存款交易）在 L1 上产生存款事件，等待 Sequencer 抓取。
并且可通过 L1 的仲裁合约进行强制包含仲裁。仲裁流程如下：

1. 用户发起存款交易，L1 上产生存款事件，时间为 t0。
2. Sequencer 承诺存款事件产生后将在一定时间内（t）获取交易。
3. 如果 Sequencer 在 t0+t 时间内没有获取交易，任意用户可以在链下生成存款事件在区块收据树中的存在证明，通过 L1 仲裁合约发起仲裁,要求 Sequencer 提供该 L2 交易包含在某个交易列表的证明。
4. 该证明可在仲裁链上验证（交易列表的根在 L1 上），如果验证失败（非法证明或 tx 所在的区块时间大于 t0+t）或者 Sequencer 无法在规定时间（t2）内提供证明，Sequencer 将被罚没一部分押金，仲裁发起者可获得相应的奖金。
