...
该方式使得 Sequencer 无法隐蔽的实现审查交易，因为任何人都可以随时验证 L1 上的合约调用与相关事件，但 L2 上交易未被包含的事实。
...
2. Sequencer 承诺 L1ToL2 事件产生后将在一定时间内（t）获取交易。
3. 如果 Sequencer 在 t0+t 时间内没有获取交易，任意用户可以在链下生成 L1ToL2 事件在区块收据树中的存在证明，通过 L1 仲裁合约发起仲裁,要求 Sequencer 提供该 L2 交易包含在某个交易列表的证明。
