...
Named objects are a convenient way for developers to obtain objects. Named objects must be globally unique.
...
Whether the `T` of `Coin<T>` needs to contain `T` is decided by the developer.
