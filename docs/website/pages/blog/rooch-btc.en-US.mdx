...
- All Bitcoin block heads and UTXO states are synced to Rooch as objects in the MoveVM, where the UTXOs are indexer and cross verified by the light client and all the block heads.
- The indexer Bitcoin UTXO information can then be used in the smart contracts of the fully on-chain application built on Rooch.
...
The RoochBTC network will have the full Bitcoin mainnet state indexer on the chain, as well as everything you need for developing a fully on-chain application with Bitcoin assets. This includes SDK, wallet API, documents, and examples.
