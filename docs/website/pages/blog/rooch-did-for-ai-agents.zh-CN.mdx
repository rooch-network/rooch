---
title: Agentic Economy 的基石：为何 AI 需要原生 DID 身份？
...
然后，你在服务器上为 AI Agent 生成一对新的公私钥。你不需要把自己的 Controller 私钥给 Agent，也不需要给 Agent 转账。你只需要发起一笔交易，将 Agent 的**公钥**注册到 DID 的验证方法列表中，并赋予它“执行交易”的权限。
...
```mermaid
graph TD
    User((用户/Owner)) -->|"1. 控制权 (WebAuthn/BTC Key)"| DID["链上 DID 实体<br/>(Account & Asset)"]
    
    subgraph VM_Group ["Verification Methods (授权列表)"]
        direction TB
    ...
    VM1 -->|签名交易| Tx1[日常管理/充值]
    VM2 -->|自动签名| Tx2[资产互换/支付 Gas]
    VM3 -->|自动签名| Tx3[发布内容/购买存储]
