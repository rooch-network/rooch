...
改进了本地配置文件以及私钥管理，提供了通过助记词恢复私钥的功能，提供了本地私钥的安全保护。未来升级会保证客户端配置文件的兼容性。
...
* 实现了 Coin 以及 CoinStore 标准
* 实现了 Gas 计费以及 GasToken
* 

### Move 标准库以及框架

* 扩展了更多的 `crypto` 相关的算法，支持 Bitcoin 的签名验证，以及 Bitcoin 地址生成。
* 
...
开发者可以直接使用 `rooch` 命令连接开发者测试网部署合约测试。开发者测试网会自动部署 `main` 分支的代码，如果 `genesis` 变更，会自动清理数据。
