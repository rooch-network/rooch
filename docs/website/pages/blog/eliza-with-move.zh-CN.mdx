...
Rooch Network 通过探索使用 Move 语言实现 Eliza 的完全链上版本来开辟新天地。这种创新方法将角色数据和内存直接存储在智能合约中，释放了真正去中心化人工智能代理的潜力。
...
数据结构遵循 Eliza 之前使用的 JSON 标准，有助于将现有字符定义无缝迁移到区块链上。如需完整代码，请访问 [Rooch GitHub 存储库](https://github.com/rooch-network/rooch/tree/main/examples/foc_eliza)。

## 链上 Eliza 入门

开发人员可以利用链上字符和内存存储与 Eliza 进行动态交互。以下是部署和使用它的快速指南：
...
将 Eliza 的角色和内存存储转移到链上可带来变革性的好处：
...
通过链上治理，社区可以定义和执行角色进化和内存管理的规则。例如：
...
在链上存储字符和内存可增强透明度和可靠性，使 Eliza 成为开放且值得信赖的人工智能生态系统的理想选择。
