---
title: Rooch v0.3 发布，启动先行测试网
description: "Rooch v0.3 发布，接入比特币网络，实现铭文铭刻，启动先行测试网。"
...
* 接入 Bitcoin 测试网络，将 UniSat 作为默认钱包，详细使用方法请参考[安装 UniSat](../docs/user-guides/install-unisat.zh-CN.mdx)。
* Rooch 实现 Bitcoin 中继网络，为成为 Bitcoin 网络执行层提供基本支持。
* 实现交易类型、交易进度、地址、余额、铭文、UTXO 等解析。

### 对象存储

这一版本对对象存储做了重大重构，详细使用请参考[对象](../docs/developer-guides/object.zh-CN.mdx)，为开发者提供更好的开发体验。
