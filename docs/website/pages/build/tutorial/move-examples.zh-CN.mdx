...
我们向 `examples` 中贡献示例时，使用 `rooch move new` 初始化项目后，`Move.toml` 文件的项目地址需要使用 `_` 通配符替换外，还需要增加一个 `[dev-addresses]` 表，填写一个用于单元测试的地址。
...
[`nostr`](https://github.com/rooch-network/rooch/tree/main/examples/nostr)

### References

[Schnorr 签名和其应用](https://rooch.network/zh-CN/blog/schnorr-signature-and-their-applications)
