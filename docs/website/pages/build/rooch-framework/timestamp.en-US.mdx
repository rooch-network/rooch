...
After starting the relay service, we can use the `rooch object --id 0x2::timestamp::Timestamp` command to obtain some information returned by the timestamp object. The returned information includes the current timestamp.
