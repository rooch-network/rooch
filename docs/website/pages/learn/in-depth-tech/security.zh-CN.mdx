...
> Rooch 还将支持[由 ZK Proof 启用的混合安全性](./hybrid-security.zh-CN.mdx)。

## flexEmu - 提供单步证明的通用模拟器

生成欺诈证明是确保 Optimistic Rollup 系统中链下系统安全的关键部分。为此，我们设计了 [flexEmu](https://github.com/rooch-network/flexemu)。

import Image from "next/image";

<Image
  src="/docs/rooch-flexemu.svg"
  alt="Rooch flexEmu"
  width={500}
  height={500}
...
> 在 GitHub 上查找 [flexEmu 的详细说明](https://github.com/rooch-network/flexemu/blob/main/docs/guidelines.md)。
...
> Tokenomics 将于 [2024 年第四季度发布](/learn/miscellaneous/roadmap)
...
- **与任意 Layer1 的兼容性**
  任何支持 MIPS 指令集的 Layer1 都可以仲裁 flexEmu 生成的证明。这使得 Rooch 成为任意 Layer1 的 Layer2。
- **易于维护、调试和升级**
  如果未来 MoveVM 有重大升级，flexEmu 可以快速容纳。
