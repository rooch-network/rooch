...
- **MoveOS** - MoveOS 是执行所有计算和存储工作的组件
  ...
  - **MoveVM** 是一个完整的功能图灵完备的 VM，用于执行用 [Move 语言](/learn/core-concepts/move-contracts/move-language)编写的智能合约
...
- **ZK Prover** - ZK Prover 根据批处理的交易生成状态转换的 ZK 证明。证明将发送到仲裁层以优化安全机制所需的挑战期。(有关更多信息，请参见使用 [Hybrid Security with ZK Proof](/learn/in-depth-tech/hybrid-security)）
