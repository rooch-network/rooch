...
This section specifies the Rooch account architecture enabled by the DID auth validator. The terminology and requirements use RFC 2119 normative keywords.

- Controller Key (External Key Store)
  - Definition: A private key held by the user in an external wallet (e.g., Passkey/WebAuthn, Bitcoin wallet, Ethereum wallet).
  - Role: Controls the DID; authorized to manage the DID Document (add/remove verification methods, rotate controllers).
  - Storage: MUST be held and safeguarded by the user’s external key store; Rooch does not custody it.

- DID Account (On-chain DID Document)
  - Definition: A smart-contract DID deployed on-chain that represents the account.
  - Provisioning: MAY be created via the cadop service or any equivalent deployment flow.
  - Binding: Controller keys MUST be registered as controllers of the DID; transactions sent by the DID MUST be authorized by verification methods listed under the DID’s `authentication` relationship.

- Application Verification Method (App VM)
  - Definition: A key pair generated by an application for a specific user DID, added as a verification method (VM) to that DID.
  - Storage: MUST be stored locally by the application; Rooch nodes never store private keys.
  - Permissions: App VM keys SHOULD be granted the `authentication` relationship to enable transaction signing on behalf of the DID; scope MAY be limited by DID policy.
  - Lifecycle: Applications MAY rotate or revoke App VM keys; controllers MUST be able to revoke any App VM immediately.

### Lifecycle

1. Registration
   - The user selects an external controller key in their wallet.
   - A DID account is deployed on-chain (e.g., via cadop).
   - The controller key is added to the DID Document as a controller; optional initial App VM(s) MAY be registered.

2. Application onboarding
   - An application generates a local key pair.
   - The application requests that the DID add the public key as a verification method with the `authentication` relationship; the request MUST be authorized by a controller or an already-authorized method per DID policy.

3. Transaction authorization
   - A transaction is signed using one of the DID’s authorized verification methods:
     - Controller key directly; or
     - App VM key.
   - The DID auth validator verifies the signature according to the chosen envelope and the DID Document state. On success, it returns the DID and VM fragment; no session key creation is required.

4. Rotation and recovery
   - Controllers MAY rotate their keys and MUST be able to revoke compromised App VM keys.
   - Recovery procedures SHOULD be defined by DID governance (e.g., multi-controller policies, social recovery).

### Security and Governance

- Authorization model: Only verification methods listed in the `authentication` relationship MAY authorize transactions.
- Least privilege: App VM keys SHOULD be scoped by DID policy when possible.
- Revocation: Removal from the DID Document MUST take immediate effect for subsequent transactions.
- Privacy: App VM keys are per-application; applications SHOULD avoid key reuse across distinct apps.

### Implications for Validators and SDKs

- The DID auth validator MUST accept signatures from any verification method that is authorized under the DID’s `authentication` relationship.
- SDKs SHOULD provide helpers to:
  - Create and register App VM keys,
  - Sign using envelopes supported by the validator,
  - Manage rotation and revocation flows.

### Account System Diagram

```mermaid
flowchart TB
  U[User]

  subgraph EKS[External Key Stores]
    PK[Passkey / WebAuthn]
    BW[Bitcoin Wallet]
    EW[Ethereum Wallet]
  end

  D[DID Account<br/>On-chain DID Document]
  ...
  A -->|generates| AK
  AK -->|registers as authentication| D
```
