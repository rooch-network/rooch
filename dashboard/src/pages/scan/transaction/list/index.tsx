...
const defaultColumns: GridColDef[] = [
    {
        ...
        renderCell: ({row}: CellType) => <Typography
            sx={{color: 'text.secondary'}}>{row.sequence_info.tx_order}</Typography>
    },
    {
        flex: 0.1,
        field: 'tx_hash',
        ...
        renderCell: ({row}: CellType) => <LinkStyled
            href={`/scan/transaction/detail/${row.execution_info.tx_hash}`}>{formatAddress(row.execution_info.tx_hash)}</LinkStyled>,
    },
    {
        flex: 0.1,
        field: 'method',
        minWidth: 80,
        headerName: 'Method',
        renderCell: ({row}: CellType) => <LinkStyled href="/">{row.transaction.action_type.toUpperCase()}</LinkStyled>,
    },
    // {
    //   flex: 0.3,
    //   field: 'age',
    ...
    // },
    {
        ...
        <Tooltip placement='bottom' sx={{cursor: 'pointer'}} title={row.transaction.sender}>
            <Typography sx={{color: 'text.secondary'}}>{formatAddress(row.transaction.sender)}</Typography>
    ...
    },
    {
        ...
        headerName: 'Status',
        renderCell: ({row}: CellType) => (
            <Typography sx={{color: 'text.secondary'}}>{row.execution_info.status.type.toUpperCase()}</Typography>
        ),
    },
    {
        flex: 0.1,
...
const TransactionList = () => {
    ...
    useEffect(() => {
        ...
        dispatch(fetchData({cursor: paginationModel.page * paginationModel.pageSize, limit: paginationModel.pageSize, dispatch}))
    ...
    return (
        <Card>
            <DataGrid
                ...
                rows={status === 'finished' ? result.data.map((row) => ({...row, id: row.execution_info.tx_hash})) : []}
